...
import ShareButton from '@/app/results/[id]/ShareButton'
import PendingResults from '@/app/results/[id]/PendingResults'
...
export default function ResultsClient({ 
  ...
  if (fetchError || !diagnosis || diagnosis.status === 'error') {
    return (
      <div className="max-w-md mx-auto text-center py-24">
        ...
        <h2 className="font-display text-xl font-semibold text-slate-900 dark:text-white mb-2">{t.resUnavTitle}</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">{t.resUnavDesc}</p>
        <Link
          href="/upload"
  ...
  return (
    <div className="max-w-4xl mx-auto pb-16">
        ...
        {diagnosis.image_url && (
          <div className="w-full sm:w-56 flex-shrink-0">
            <img
              ...
              alt="Plant photo"
              className="w-full sm:w-56 h-52 object-cover rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm"
            />
          </div>
        )}
        <div className="flex-1 min-w-0 pt-1">
          <p className="text-[10px] font-bold uppercase tracking-widest text-green-600 dark:text-green-400 mb-2">{t.resComplete}</p>
          <h1 className="font-display text-3xl sm:text-4xl font-semibold text-slate-900 dark:text-white leading-tight mb-3">
      ...
      {/* Research disclaimer */}
      <div className="rounded-xl bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-800 px-5 py-4">
        <SourceNote />
      </div>
    </div>
  )
